...
void RCC_INIT (void){
	...
	while((RCC->CR & RCC_CR_PLLRDY) == RCC_CR_PLLRDY);		 																												          // Ждем отключения PLL
	...
	RCC->PLLCFGR = ((RCC->PLLCFGR & ~RCC_PLLCFGR_PLLN) | RCC_PLLCFGR_PLLN_3 | RCC_PLLCFGR_PLLN_5 | RCC_PLLCFGR_PLLN_7);   // PLLN PRE SET to 168
	RCC-> PLLCFGR &= ~RCC_PLLCFGR_PLLP;   																																									 // PLLP PRE SET to 2
		
	RCC->CR |= RCC_CR_PLLON;																																																	 	 // PLL ON
	while ((RCC->CR & RCC_CR_PLLRDY) == 0);
	
	RCC->CFGR = ((RCC->CFGR & ~RCC_CFGR_SW) | RCC_CFGR_SW_PLL);                                                          // System Clock set tot PLLCLK
